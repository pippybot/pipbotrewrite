private setupBotBehavior() {
    this.twitchClient.on('message', (channel: any, tags: any, message: any, self: any) => {
        const helloCommand = "Hey there, $(user)! Welcome to the stream! Please make sure to read the rules by typing '!rules'.";

        // Ignore messages from itself.
        if (self) return;

        //! means a command is coming by, and we check if it matches the command we currently support
        if ((message.startsWith('pineapple pizza') || message.startsWith('Pineapple pizza') || message.startsWith('Pineapple Pizza') && message === helloCommand) {
